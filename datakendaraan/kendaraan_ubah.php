...
$queryEdit=mysql_query("SELECT * FROM kendaraan WHERE id_kendaraan='$idKendaraan'");
$hasilQuery=mysql_fetch_array($queryEdit);
$idKendaraan=$hasilQuery['id_kendaraan'];
$idSupir=$hasilQuery['id_supir'];
$idPemandu=$hasilQuery['id_pemandu'];
$nopol=$hasilQuery['nopol'];
$jenisKendaraan=$hasilQuery['jenis_kendaraan'];
$perusahaan=$hasilQuery['perusahaan'];
				...
				<div class="col-md-12">
					...
					<form action="aksi_edit.php" method="post">
						<div class="row form-group">
							...
							<select class="form-control" name="idSupir">
							<?php
							$kueriSupir = mysql_query("select * from supir");
							while ($spr=mysql_fetch_array($kueriSupir))
							{?>
							<option value="<?php echo $spr['id_supir']; ?>"><?php echo $spr['nama_supir'];?></option>
							<?php } ?></select>
							</div>
						</div>
							...
							<div class="row form-group">
							<div class="col-md-12">
							<select class="form-control" name="idPemandu">
							<?php
							$kueriPemandu = mysql_query("select * from pemandu");
							while ($pmd=mysql_fetch_array($kueriPemandu))
							{?>
							<option value="<?php echo $pmd['id_pemandu']; ?>"><?php echo $pmd['nama_pemandu'];?></option>
							<?php } ?></select>
							</div>
						</div>
						...
						<div class="row form-group">
							<div class="col-md-12">		
								<label class="sr-only" for="name">Nopol</label>
								<input type="text" id="nopol" name="nopol" class="form-control" value="<?php echo $nopol;?>" >
							</div>
							...
							<div class="row form-group">
							<div class="col-md-12">
								<label class="sr-only" for="name">Jenis Kendaraan</label>
								<input type="text" id="jenisKendaraan" name="jenis_kendaraan" class="form-control" value="<?php echo $jenisKendaraan;?>" >
							</div>
						...
						<div class="row form-group">
							<div class="col-md-12">
								...
								<input type="text" id="deskripsi" name="deskripsi" class="form-control" value="<?php echo $perusahaan;?>" >
							</div>
